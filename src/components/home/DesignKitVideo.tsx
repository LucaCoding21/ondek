...
// no way to replay it.
const SRC =
  "https://player.vimeo.com/video/521215844?title=0&byline=0&portrait=0&dnt=1&autoplay=1&muted=1&loop=1&controls=0&playsinline=1";
...
export default function DesignKitVideo() {
  ...
  useEffect(() => {
    function onMessage(event: MessageEvent) {
      ...
      if (data.event === "timeupdate") {
        ...
        if (seconds < duration - BLACK_TAIL_SECONDS) {
  ...
  return (
    // Overscan by 2%: the source is 1.775:1, just off 16:9, and the rounding
    // gap renders as a black seam along the top edge.
    // Fills whatever box the caller sizes — the section derives that from the
    // viewport height so the whole composition fits one screen.
